...
def get_optimality_and_batch(batch_power: int):
    ...
    # Train model
    print("Training model number {}".format(batch_power))
    ...
    # Plot heatmap
    plt.figure(num=batch_power, figsize=(8, 6))
    sns.heatmap(pdf, cmap='jet')
    plt.xticks(np.linspace(0, 9, 10, dtype=int), fontsize=16)
    plt.savefig('Figures/heat_map_{}.pdf'.format(batch_power), format='pdf', bbox_inches='tight')
    plt.show()
    ...
    return batch_power, uf.elementary_curve_optimality(curve=results, levels=decomposition_levels)
...
if __name__ == '__main__':
    ...
    for batch_power in np.linspace(7, 14, 7, dtype=int):
        b, o = get_optimality_and_batch(batch_power=batch_power)
        batches.append(b)
        optimality.append(o)
        print("run: {}, batch_size: {}, optimality: {}".format(batch_power, batches, optimality))

    df = pd.DataFrame({'batch_size': batches, 'optimality': optimality})
    df.to_csv('Experiments_results/batch_optimality.csv', index=False)

    plt.figure(num=99, figsize=(8, 6))
    plt.plot(batches, optimality, linewidth=3, color='blue')
    plt.xticks(size=16)
    plt.yticks(size=16)
    plt.savefig('Figures/batch_optimality.pdf', format='pdf', bbox_inches='tight')
    plt.show()
